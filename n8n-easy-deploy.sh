...
# Source modular scripts – ensure they are present
...
show_menu() {
  while true; do
    # Clear terminal for a clean menu (tput reset works on most terminals)
    tput reset
    ...
    case "$choice" in
      1)
        deploy || { log "Deployment failed."; sleep 3; }
        ;;
      2)
        update_instance || { log "Update failed."; sleep 3; }
        ;;
      3)
        backup || { log "Backup failed."; sleep 3; }
        ;;
      4)
        restore || { log "Restore failed."; sleep 3; }
        ;;
      5)
        check_and_repair || { log "Check and repair encountered errors."; sleep 3; }
        ;;
      6)
        secure_server || { log "Security hardening failed."; sleep 3; }
        ;;
      7)
        deploy_local || { log "Local deployment failed."; sleep 3; }
        ;;
      8)
        log "Exiting n8n Easy Deploy." && exit 0
        ;;
      *)
